...
const CartaUtente carteAutorizzate[] PROGMEM = {
...
void loop() {
  ...
  for (int i = 0; i < numeroCarte; i++) {
    ...
    for(int j=0; j<4; j++) {
      uidTemp[j] = pgm_read_byte(&(carteAutorizzate[i].uid[j]));
    }

    if (confrontaUID(rfid.uid.uidByte, uidTemp)) {
      idTrovato = pgm_read_word(&(carteAutorizzate[i].azioneID));
      deckTrovato = pgm_read_word(&(carteAutorizzate[i].numberDeck));
      break;
    }
  }
